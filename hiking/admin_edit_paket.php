...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($kuota_min > $kuota_max) {
        ...
        if ($conn->query($query_update) === TRUE) {
            echo "<script>alert('Paket berhasil diperbarui!'); window.location.href='admin_manage_paket.php';</script>";
        } else {
            echo "<script>alert('Gagal memperbarui paket: " . $conn->error . "');</script>";
        }
    }
}
?>
...
<div class="container mt-5">
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control" id="harga" name="harga" value="<?php echo htmlspecialchars($paket['harga']); ?>" required>
        </div>
        <div class="mb-3">
            <label for="deskripsi" class="form-label">Deskripsi</label>
            <textarea class="form-control" id="deskripsi" name="deskripsi" rows="3" required><?php echo htmlspecialchars($paket['deskripsi']); ?></textarea>
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="durasi" name="durasi" value="<?php echo htmlspecialchars($paket['durasi']); ?>" required>
        </div>
        <div class="row">
            <div class="col-md-6 mb-3">
                <label for="kuota_min" class="form-label">Kuota Minimal</label>
                <input type="number" class="form-control" id="kuota_min" name="kuota_min" value="<?php echo htmlspecialchars($paket['kuota_min']); ?>" required>
            </div>
            <div class="col-md-6 mb-3">
                <label for="kuota_max" class="form-label">Kuota Maksimal</label>
                <input type="number" class="form-control" id="kuota_max" name="kuota_max" value="<?php echo htmlspecialchars($paket['kuota_max']); ?>" required>
            </div>
        </div>
